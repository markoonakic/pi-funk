...
The extension consolidation, standalone-repository cleanup, and all pre-Herdr/Nix work are complete. The remaining work is governed by these decisions:

1. **Herdr and Nix are the next and final phase, deferred until the user resumes tomorrow.** The approved scope is Herdr 0.7.0 → 0.7.4, the official current Pi integration under Nix/Home Manager ownership, removal of `herdr-pi-session-name-sync.ts`, preservation/reconciliation of the existing Pi 0.80.10 pin dirt, a rebuild, and smoke tests for reload/resume, lifecycle reporting, and subagent isolation.
