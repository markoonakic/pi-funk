...
1. **Herdr and Nix finalization is complete.** The upstream Herdr flake input remains floating in `flake.nix` and reproducible through `flake.lock`; Herdr 0.7.4 with protocol 17 is active. The official Pi hook v6 is vendor/installer-managed and ignored, the custom name-sync hook was removed, and the Pi 0.80.10 derivation was reconciled. Structural restore succeeded, but only panes with valid persisted root-session references auto-resume.
