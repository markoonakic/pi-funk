...
- `npm:pi-dynamic-workflows` — Adds the `workflow` tool for Claude-Code-style dynamic workflows: the model writes a deterministic JavaScript script using `agent()`, `parallel()`, `pipeline()`, `phase()`, and `log()` to run in-memory Pi agents with fan-out/fan-in. Installed globally for experimentation; treat as prototype/orchestration-adjacent and prefer bounded/read-only use until its concurrency, tool inheritance, cancellation, and interaction with `pi-subagents` are validated.
